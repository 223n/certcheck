...
残日数は時間の端数を切り捨てた値なので、既定値の0では期限切れの当日、残り24時間を切った時点から通知されます。
...
`master`へのpushとpull request、および毎週月曜に、GitHub Actionsが次を実行します。

1. `gofmt`、`go vet`、`go test -race -cover ./...`
1. `govulncheck`による脆弱性スキャン
1. Windows、Linuxそれぞれのamd64、386向けクロスビルド

週次で回すのは、依存が変わらなくても脆弱性データベース側が更新されるためと、
時間の経過で壊れるテストをpushを待たずに検出するためです。

### Go 1.26への引き上げに伴うTLSの挙動変更

`go.mod`のGoディレクティブは、ビルド結果に焼き込まれるGODEBUGの既定値を
決めます。1.24から1.26に上げたことで、TLSクライアントとしての挙動が3点
変わります。certcheckはTLSハンドシェイクだけを行うツールなので、いずれも
監視結果に直接影響します。

| 設定 | 1.24 | 1.26 | 影響 |
| --- | --- | --- | --- |
| `tlssha1` | 1 | 0 | TLS 1.2でSHA-1署名を拒否する（Go 1.25、RFC 9155） |
| `tlssecpmlkem` | 0 | 1 | 耐量子鍵交換X25519MLKEM768を既定で提示する（Go 1.24） |
| `urlstrictcolons` | 0 | 1 | URLのコロンの解釈が厳しくなる（Go 1.26） |

前2者に該当する古いエンドポイントは、残日数ではなくハンドシェイク失敗として
`NG`で通知されます。とくに`tlssecpmlkem`は、大きくなったClientHelloを扱えない
中間機器がある場合に効きます。

監視対象にそうした機器がある場合は、`main.go`に次の行を置くと個別に従来の
挙動へ戻せます。

```go
//go:debug tlssha1=1
//go:debug tlssecpmlkem=0
```

上記の既定値は`debug.ReadBuildInfo()`の`DefaultGODEBUG`を両バージョンで
ビルドして実測したものです。
...
テスト（`go vet`、`go test -race -cover`、`govulncheck`）を通ったときだけ、
タグの作成、リリースノートの生成、実行ファイルと`SHA256SUMS`の添付まで自動で
行います。
